...
export const clarifyThenExecuteTask: RuntimeEvalTask<ClarifyThenExecuteWorld> = {
  ...
  evaluate(input) {
    if (input.turns.length !== 2)
      return { passed: false, code: "clarify_execute.turn_count", message: "Expected two completed turns." };
    ...
    if (!JSON.stringify(input.session.lines).includes(input.world.answer))
      return {
        passed: false,
        code: "clarify_execute.answer_persistence",
        message: "The scripted answer was not persisted in the session tool result.",
      };

...
- Border and frame art reused at several sizes — popup backgrounds, panels, speech bubbles, button frames — needs `ScaleType = "Slice"` with `SliceCenter` in source-image pixels, not plain stretching. Ask for the border thickness and the source image size when the user has not given them.

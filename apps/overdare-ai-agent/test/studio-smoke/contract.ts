...
export const STUDIO_SHIPPING_EXE_RELATIVE_PATH = String.raw`Sandbox\Binaries\Win64\Sandbox-Win64-Shipping.exe`;
...
export function readSmokeContract(env: NodeJS.ProcessEnv = process.env): SmokeContract {
  ...
  const windowsRuntimeDir = required(env, "OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR");
  if (!win32.isAbsolute(windowsRuntimeDir)) {
    throw new Error("OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR must be an absolute Windows path");
  }
  ...
  return {
    source,
    studioSha256,
    windowsRuntimeDir: win32.normalize(windowsRuntimeDir),
    studioArgs,
    studioRpcPort,

...
const VALID_ENV: NodeJS.ProcessEnv = {
  ...
  OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR: String.raw`C:\runtime`,
...
describe("Studio smoke contract", () => {
  ...
  test("validates the explicit Studio launch contract", () => {
    ...
    expect(contract.windowsRuntimeDir).toBe(String.raw`C:\runtime`);
  ...
  test("requires an explicit absolute Windows runtime directory", () => {
    expect(() =>
      readSmokeContract({
        ...VALID_ENV,
        OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR: undefined,
      }),
    ).toThrow("OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR");
    expect(() =>
      readSmokeContract({
        ...VALID_ENV,
        OVERDARE_STUDIO_WINDOWS_RUNTIME_DIR: "relative/runtime",
      }),
  ...
  test("always opens the staged Studio map before applying extra arguments", () => {
    ...
    expect(
      createStudioLaunchArgs(JSON.parse(VALID_ENV.OVERDARE_STUDIO_ARGS_JSON!), values, { unattended: true }),
    ).toEqual([
    ...
    expect(createStudioLaunchArgs([], values)).not.toContain("-unattended");
  });
});

...
class FakeWebSocket {
  ...
  constructor(public readonly url: string) {
    ...
    if (FakeWebSocket.openDelayMs > 0) {
      setTimeout(open, FakeWebSocket.openDelayMs);
    } else {
      queueMicrotask(open);
    }
  }
...
test("waits for an in-progress connection before sending a request", async () => {
  FakeWebSocket.openDelayMs = 20;
  globalThis.WebSocket = FakeWebSocket as unknown as typeof WebSocket;
  ...
  ).resolves.toMatchObject({ serverName: "fake" });
...
test("uses an explicit development RPC URL when configured", () => {
  expect(resolveWebSocketUrl({ protocol: "http:", host: "localhost:5174" }, "ws://127.0.0.1:7433/rpc")).toBe(
    "ws://127.0.0.1:7433/rpc",
  );
  expect(resolveWebSocketUrl({ protocol: "https:", host: "example.test" }, undefined)).toBe("wss://example.test/rpc");
});

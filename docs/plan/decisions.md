...
### D104: `@diligent/logging` is a dependency-free leaf with a split record/sink contract
- **Decision**: `@diligent/logging` is positioned as a dependency-free leaf package below `core`, `runtime`, and all host applications. The package owns the record contract (the `LogRecord` type, scope/event/message/fields shape, child logger API, console formatting, level filtering, and fanout). Hosts own all sinks (console, file, remote, telemetry). Correlation context (`sessionId`, `threadId`, `turnId`) is carried as structured fields on each record, not as process-global state. Logs are not protocol messages and must never be routed through the protocol layer.
- **Rationale**: A cross-cutting diagnostic concern that imports from `core` or `runtime` would create a circular or reversed dependency. Placing the logging contract in a zero-dependency leaf lets every layer — including `core` itself — emit structured records without introducing cycles. The host-owns-sink split means product-specific output destinations (OVERDARE telemetry, remote log aggregation, TUI stderr) are assembled at the composition root without infecting the shared record contract. Process-global current-session state is prohibited because hosts run concurrent sessions; scoping correlation to record fields is the only safe approach.
- **Alternatives considered**: Inline structured logging into each package independently (inconsistent schemas, no shared fanout); include `@diligent/logging` as part of `core` (reverses the dependency hierarchy for packages that currently have no `core` dep); use a process-global singleton for correlation (breaks concurrent sessions and makes test isolation impossible).
- **Consequence**: Any new package that needs structured logging adds `@diligent/logging` as a direct dependency — not `@diligent/core` or `@diligent/runtime`. `@diligent/logging` must remain free of runtime or core imports. Hosts are responsible for registering and shutting down sinks; sink failures must never alter agent control flow. User-visible CLI output, protocol frames (JSON-RPC, MCP), and TUI rendering text must not be routed through the diagnostic logger.
- **References**: `docs/guide/logging.md`, ARCHITECTURE.md "Logging vs Protocol" boundary bullet, `packages/logging/src/`
- **Date**: 2026-07-15
